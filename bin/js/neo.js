...
var Neo = function(_arguments) {
    ...
    this.select = function(selector, identifier, parent) {
        ...
        if(!global.isDefined(selector) || global.isNull(selector)) {
            return undefined;
        }

        if(self.isArray(selector) || self.isElementCollection(selector)) {
            return new nodeList(self.toNodes(selector));
        }

        return global.isNode(selector) ? selector : new node(selector);
     ...
     * @returns {node}
     */
    this.create = function(tag, inner) {
        var node = global.select(document.createElement(tag));
        if(global.isDefined(inner)) {
            node.inner(inner);
        }
        return node;
    };
    ...
    this.isRetina = function() {
        ...
        if(global.scaleRatio() > 1) {
            return true;
        }

        if(global.matchMedia("(-webkit-min-device-pixel-ratio: 1.5), (min--moz-device-pixel-ratio: 1.5), (-o-min-device-pixel-ratio: 3/2), (min-resolution: 1.5dppx)")) {
            return true;
        }
     ...
     */
    this.isIE = function() {
        return '\v' == 'v' || /msie/i.test(navigator.userAgent);
    };
    ...
    var node = function(_origin) {
        ...
        this.bind = function(_events, _callback, save) {
            ...
            if(_arguments['enable_history'] && save) {
                self.history().save();
            }
         ...
         * @returns {Neo.node}
         */
        this.addClass = function(_className) {

            var classList = self.origin.classList;
            var className = self.origin.className;
            ...
            if(global.isDefined(classList)) {
                ...
                if(className.length == 0) {
                    self.origin.className = _className;
                } else {
                    if(className.indexOf(" ") != -1) {
                        if(className.indexOf(className + " ")) {
                            return self;
                        }
                    } else if(className.indexOf(className)) {
                        return self;
                    }
                    self.origin.className += " " + _className;
                }
        ...
        this.hasClass = function(_className) {
            ...
            if(global.isDefined(classList)) {
                ...
                return self.origin.className.match(new RegExp('(\\s|^)' + _className + '(\\s|$)'));
            }
        ...
        this.removeClass = function(_className) {
            ...
            if(global.isDefined(classList)) {
                ...
                self.origin.className = self.origin.className.replace(regex, " ");
            }
         ...
         * @returns {Neo.node}
         */
        this.toggleClass = function(_className) {
        ...
        this.after = function(_node) {
            self.origin.parentNode.insertBefore(self.origin, self.origin.nextSibling);
            return self;
        };
    ...
    var history = function(_node) {
        ...
        this.revert = function(indices) {
            ...
            if(indices <= 0) {
                throw "The amount of indices you'regex going back can't equal null or less.";
            }
